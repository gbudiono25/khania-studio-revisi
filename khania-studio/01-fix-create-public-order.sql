CREATE OR REPLACE FUNCTION public.create_public_order(p_full_name text, p_business_name text, p_whatsapp text, p_email text, p_domain text, p_package_code text, p_voucher_code text DEFAULT NULL::text)
 RETURNS TABLE(success boolean, order_id uuid, order_number text, client_id uuid, client_code text, package_name text, base_price bigint, setup_fee bigint, voucher_code text, voucher_discount bigint, total_amount bigint, due_date timestamp with time zone, status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
