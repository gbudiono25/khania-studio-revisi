document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load packages from API (Supabase), with hardcoded fallback
  let packages = {};
  try {
    ...
    console.warn('Khania Studio pemesanan.js: API packages unavailable, using fallback.', e);
  }

  // Fallback to spec-defined packages
  ...
  // Voucher validation via server-side API (Supabase-backed)
  async function applyVoucher(){
    ...
    try {
      const res = await fetch(`api/validate-voucher.php?code=${encodeURIComponent(code)}&package=${encodeURIComponent(pkg.name)}`, {cache:'no-store'});
      const data = await res.json();
      if (!data.valid){
        voucherMessage.textContent = data.message || 'Kode voucher tidak valid atau tidak tersedia.';
        voucherMessage.className='hint voucher-error';
        renderTotal();
        return;
      }
      const totalSubtotal = pkg.base + pkg.setup;
      if (data.discount_type === 'percent') {
        discount = Math.round(totalSubtotal * Math.min(100, Number(data.discount_value) || 0) / 100);
      } else {
        discount = Math.min(totalSubtotal, Number(data.discount_value) || 0);
      }
      $('voucherCodeInput').value = code;
      voucherStatus.textContent = '('+code+')';
      voucherMessage.textContent = data.message || '✓ Voucher berhasil digunakan.';
